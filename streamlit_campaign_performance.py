...
st.set_page_config(page_title="Campaign Incrementality — Parallel-scale", layout="wide")
...
# -------------------------
# Cleaning & imputation (user function, robustified)
# -------------------------
def clean_and_impute(df,
    ...
    if carpet_col in df.columns and carpet_col in df.columns:
...
# -------------------------
# Pivot & control selection helpers
# -------------------------
...
# -------------------------
# Expected-by-scale (level scaling, control-sum)
# -------------------------
def expected_by_scale(pivot, target, controls, pre_start, pre_end, meas_start, meas_end, level_method='mean', eps=1e-9):
    """
    pivot: Date-indexed pivot table columns=brands
    target: brand string
    controls: list of control brand strings (summed)
    ...
    # slice full window (pre + measurement)
    full_start = pd.to_datetime(pre_start)
    full_end = pd.to_datetime(meas_end)
    ...
    # require data for pre & measurement
    ...
    # expected = scale * control_sum across full window (pre+meas)
...
# Plot helpers
# -------------------------
def plot_target_full(target_name, target_full, expected_full, control_full, pre_start, meas_end, campaign_start, campaign_end):
    fig = go.Figure()
...
def plot_combined_full(actual_comb, expected_comb, control_sum, pre_start, meas_end, campaign_start, campaign_end):
    fig = go.Figure()
...
def make_campaign_html(campaign_meta, combined_fig, per_target_figs, per_target_metrics):
    ...
    parts.append(f"<li>PrePeriod: {campaign_meta['PrePeriod Start'].date()} to {campaign_meta['PrePeriod End'].date()}</li>")
    ...
    for t in campaign_meta['Targets']:
        ...
        if met:
            parts.append("<ul>")
            parts.append(f"<li>Actual (meas total): {met.get('actual_total', 0):.2f}</li>")
            parts.append(f"<li>Expected (meas total): {met.get('expected_total', 0):.2f}</li>")
...
# Streamlit UI
# -------------------------
st.title("Campaign Incrementality — Parallel-scale method")

st.markdown("Upload sales CSV (must contain `Date`, `Brand Name`, and either `Value`/`sales` or `sales_imputed`). App will clean and impute if needed.")
...
with colA:
    ...
    with st.form("camp_form"):
        ...
        camp_start = st.date_input("Campaign start (inside measurement)", key="cs")
        ...
        if pre_mode == "Manual":
            pre_start_manual = st.date_input("Pre-period start", key="pre_start_manual")
            pre_end_manual = st.date_input("Pre-period end", key="pre_end_manual")
...
if st.button("Generate reports for queued campaigns"):
    ...
    if not campaigns_to_run:
        st.error("No campaigns to run. Add or upload campaign list.")
    else:
        summary_rows = []
        ...
        for camp in campaigns_to_run:
            ...
            try:
                ...
                st.error(f"Invalid campaign dates/inputs: {e}")
                continue
            ...
            if not (meas_start <= camp_start <= camp_end <= meas_end):
                st.warning("Campaign period not fully inside measurement period. Proceeding but verify inputs.")
            ...
            for t in targets:
                ...
                if combined_actual is None:
                    combined_actual = out['target_full'].reindex(out['expected_full'].index).fillna(0.0)
                else:
                    combined_actual = combined_actual.add(out['target_full'].reindex(combined_actual.index.union(out['target_full'].index)).fillna(0.0), fill_value=0.0)
                if combined_expected is None:
                    combined_expected = out['expected_full'].reindex(out['target_full'].index).fillna(0.0)
                else:
                    combined_expected = combined_expected.add(out['expected_full'].reindex(combined_expected.index.union(out['expected_full'].index)).fillna(0.0), fill_value=0.0)
                control_sum_for_combo = out['control_full'] if control_sum_for_combo is None else control_sum_for_combo.add(out['control_full'], fill_value=0.0)

            if combined_actual is None or combined_expected is None:
                st.error("No valid target data for this campaign. Skipping.")
                continue

            # restrict combined to pre..meas_end and reindex
            ...
            # Show per-target details & allow per-target downloads
            ...
            # Bundle everything into one HTML per campaign
            campaign_meta = {
